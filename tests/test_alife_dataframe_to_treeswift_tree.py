...
'''
`alife_dataframe_to_treeswift_tree` tests for
...
def test():
    ...
    converted_tree = apc.alife_dataframe_to_treeswift_tree(original_df)
    reconverted_df = apc.treeswift_tree_to_alife_dataframe(converted_tree)
...
def test_empty():
    ...
    assert apc.alife_dataframe_to_treeswift_tree(df) is None
...
def test_setattrs_iterable():
    ...
    converted_tree = apc.alife_dataframe_to_treeswift_tree(
        original_df,
        setattrs=['num_orgs'],
    )
    for node in converted_tree.traverse_postorder():
